...
namespace WindowsApp::Compute::Kernels
{
        ...
        int offsetX, int offsetY)
    {
        ...
        float srcW = invH[6] * dstX + invH[7] * dstY + invH[8];

        int dstIdx = (y * dstW + x) * 3;

        if (fabsf(srcW) < 1e-10f)
        {
            dstData[dstIdx] = 0;
            dstData[dstIdx + 1] = 0;
            dstData[dstIdx + 2] = 0;
    ...
    __device__ void SortNetwork8(unsigned short* arr)
    {
        ...
        #define SWAP(a, b) if (arr[a] > arr[b]) { unsigned short t = arr[a]; arr[a] = arr[b]; arr[b] = t; }
        SWAP(0, 1); SWAP(2, 3); SWAP(4, 5); SWAP(6, 7);
        SWAP(0, 2); SWAP(1, 3); SWAP(4, 6); SWAP(5, 7);
        SWAP(1, 2); SWAP(5, 6); SWAP(0, 4); SWAP(1, 5);
        SWAP(2, 6); SWAP(3, 7); SWAP(2, 4); SWAP(3, 5);
    ...
    __device__ void SortNetwork16(unsigned short* arr)
    {
        ...
        SWAP(1,2); SWAP(5,6); SWAP(9,10); SWAP(13,14);
        SWAP(0,4); SWAP(1,5); SWAP(2,6); SWAP(3,7); SWAP(8,12); SWAP(9,13); SWAP(10,14); SWAP(11,15);
        SWAP(2,4); SWAP(3,5); SWAP(10,12); SWAP(11,13);
        SWAP(1,2); SWAP(3,4); SWAP(5,6); SWAP(9,10); SWAP(11,12); SWAP(13,14);
        ...
        float sigmaThreshold)
    {
        ...
        if (count <= 8)
        {
            ...
            for (int i = count; i < 8; i++) values[i] = values[count - 1];
            SortNetwork8(values);
        }
        else if (count <= 16)
        {
            for (int i = count; i < 16; i++) values[i] = values[count - 1];

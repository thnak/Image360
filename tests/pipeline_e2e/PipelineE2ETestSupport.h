// Shared full-pipeline integration test body, parameterized on which
// IComputeBackend drives Ingest/Align/Optimize/Render. Loads 3 synthetic RAW
// (DNG) tiles cropped with known overlap from one master scene (see
...
// 4 stages (Ingest -> Align -> Optimize -> Render), and checks that
// Optimize's bundle adjustment actually recovers those known offsets - not
// just that the pipeline runs without crashing.
//
// Included from exactly one .cpp per test binary (tests/pipeline_e2e/main.cpp
// for the CPU backend, tests/pipeline_e2e_vulkan/main.cpp for VulkanPipeline)
// so the anonymous-namespace symbols below never collide across binaries.
//
...
namespace WindowsApp { namespace Testing
{
    namespace fs = std::filesystem;

    namespace PipelineE2EDetail
    {
        ...
        // pipeline to completion using the caller-supplied (already
        // Initialize()'d) compute backend. Leaves projectManager/storageEngine
        // open so the caller can inspect the result (homographies, chunks,
        // tasks).
        inline bool SetupAndRunPipeline(const fs::path& tempDir, const fs::path& fixturesDir,
                                         int totalWidth, int totalHeight, int chunkSize,
                                         const std::shared_ptr<Compute::IComputeBackend>& computeBackend,
                                         Core::ProjectManager& projectManager, Core::StorageEngine& storageEngine)
        {
        ...
        inline void RunSingleChunkScenario(const fs::path& fixturesDir,
                                            const std::shared_ptr<Compute::IComputeBackend>& computeBackend)
        {
            using namespace WindowsApp::Core;
            ...
            bool ranOk = SetupAndRunPipeline(tempDir, fixturesDir, 800, 280, 1024, computeBackend, projectManager, storageEngine);
        ...
        inline void RunMultiChunkScenario(const fs::path& fixturesDir,
                                           const std::shared_ptr<Compute::IComputeBackend>& computeBackend)
        {
            using namespace WindowsApp::Core;
            ...
            bool ranOk = SetupAndRunPipeline(tempDir, fixturesDir, 800, 280, kChunkSize, computeBackend, projectManager, storageEngine);
        ...
        inline std::vector<unsigned char> ReadFileBytes(const fs::path& path)
        {
        ...
        inline double ComputeMse(const unsigned char* a, int aStride, const unsigned char* b, int bStride,
        ...
        inline void RunPanoramaQualityScenario(const fs::path& fixturesDir,
                                                const std::shared_ptr<Compute::IComputeBackend>& computeBackend)
        {
            using namespace WindowsApp::Core;
            using namespace WindowsApp::Compute;
            ...
            bool ranOk = SetupAndRunPipeline(tempDir, fixturesDir, kTotalWidth, kTotalHeight, 1024,
                                              computeBackend, projectManager, storageEngine);
    ...
    // Runs all 3 pipeline e2e scenarios against fixturesDir using
    // computeBackend (already Initialize()'d by the caller) for every
    // pipeline stage, and returns the process exit code (0 = all passed).
    inline int RunPipelineE2ETests(const fs::path& fixturesDir,
                                    const std::shared_ptr<Compute::IComputeBackend>& computeBackend)
    {
        using namespace PipelineE2EDetail;

        RunSingleChunkScenario(fixturesDir, computeBackend);
        RunMultiChunkScenario(fixturesDir, computeBackend);
        RunPanoramaQualityScenario(fixturesDir, computeBackend);
